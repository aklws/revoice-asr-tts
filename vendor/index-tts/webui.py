...
if missing:
    logger.info(
        "模型目录 {} 不完整，缺少文件: {}。正在下载 IndexTTS-2 模型...",
        cmd_args.model_dir,
        ", ".join(missing),
    )
    try:
        cmd_args.model_dir = snapshot_download("IndexTeam/IndexTTS-2", local_dir=cmd_args.model_dir)
    except Exception as e:
        logger.error("下载模型到 {} 失败: {}", cmd_args.model_dir, e)
        sys.exit(1)
    missing = [f for f in required_files if not os.path.exists(os.path.join(cmd_args.model_dir, f))]
    if missing:
        logger.error(
...
def gen_single(emo_control_method,prompt, text,
    ...
    logger.info("情感控制模式: {}, 权重: {}, 向量: {}", emo_control_method, emo_weight, vec)
...
<a href='https://arxiv.org/abs/2506.21619'><img src='https://img.shields.io/badge/ArXiv-2506.21619-red'></a>
    ...
    def on_example_click(example):
        logger.info("已点击示例，共 {} 个字段: {!r}", len(example), example)
        return (
    ...
    def on_demo_load():
        ...
        try:
            ...
            logger.exception("页面加载时重新加载词汇表失败: {}", e)
        return (gr.update(value=format_glossary_markdown()),
